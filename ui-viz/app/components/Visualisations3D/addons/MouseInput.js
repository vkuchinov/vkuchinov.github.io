...
class MouseInput extends Module {
  ...
  setup(react3RendererInstance) {
    ...
    const Object3DDescriptor =
      react3RendererInstance.threeElementDescriptors.object3D.constructor;

    Object.values(react3RendererInstance.threeElementDescriptors).forEach(
      elementDescriptor => {
  ...
  _updateEnterLeave() {
    ...
    for (let depth = 0; depth < intersections.length; ++depth) {
      ...
      if (this._hoverObjectMap[uuid]) {
        ...
        if (
          !(
            mouseEnterEvent.isDefaultPrevented() ||
    ...
    for (let i = 0; i < unseenUUIDs.length; ++i) {
      ...
      if (
        !(
          mouseLeaveEvent.isDefaultPrevented() ||
          mouseLeaveEvent.isPropagationStopped()
        )
      ) {
        React3.eventDispatcher.dispatchEvent(
          this._hoverObjectMap[uuid].object,
          'onMouseLeave',
          mouseLeaveEvent
        );
      }
