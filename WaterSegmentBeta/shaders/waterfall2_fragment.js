...
void main( void ) {
    ...
    vec2 uv = (gl_FragCoord.xy/resolution.x) - (1.0,resolution.y/resolution.y);
	vec2 q = -uv;

    float strength = floor(20.);

    float T3 = max(3.,1.25 * strength) * time * 0.03 + pow(abs(q.y), 1.25) *2.; 
    float n = fbm(vec2(strength*q.x,strength*q.y) - vec2(0,T3));
    
    strength=26.;
    float T3B = max(3.,1.25*strength)*time*0.03+pow(abs(q.y),1.25)*2.;
    n = n*0.5 + (n*0.5)/(0.001+1.5*fbm(vec2(strength*q.x,strength*q.y) - vec2(0,T3B)));
    
    float intensity = abs(sin(time*0.04));
